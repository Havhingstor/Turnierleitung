...
public class HFProtagonisten {
	...
	public void aktualisiere(Steuerung steuerung, boolean other) {
		...
		if (steuerung.getTeams().size() > 0) {
			...
			Text fehlerText = new Text(
					"Es ist kein Team eingetragen. Deshalb können keine Spieler " + "hinzugefügt werden!");
	...
	private GridPane createTeamInhalt() {
		...
		/*
		 * neu.setOnAction((e)->{ new TeamHinzufuegen(stage, akt, steuerung,new
		 * Handler<Team>() { public void handle(Team t) { letztesTeamTeams=t.getID(); }
		 * }); });
		 */
		neu.setOnAction((e) -> {
		...
		loe.setOnAction((e) -> {
			if (steuerung.getTeams().size() > 0) {
				new ListDialog<Team>(steuerung.getTeams(), stage, "Welches Team soll gelöscht werden", "Team löschen",
						(f) -> {
							Hauptfenster.getAlert(AlertType.CONFIRMATION, "ACHTUNG!", null,
									"Wollen Sie dieses Team wirklich löschen?", stage, ButtonType.YES,
									ButtonType.CANCEL).showAndWait().ifPresent((answer) -> {
										if (answer == ButtonType.YES) {
											try {
												steuerung.removeTeam(f.getID());
											} catch (IllegalArgumentException iae) {
												Hauptfenster.getAlert(AlertType.ERROR, "Fehler", null, iae.getMessage(),
														stage).showAndWait();
											}
										}
									});
				...
				Hauptfenster.getAlert(AlertType.ERROR, "Fehler", null, "Es ist kein Team vorhanden!", stage)
						.showAndWait();
			}
		});
	...
	private GridPane createSpielerInhalt() {
		...
		if (letztesTeamSpieler == 0) {
			...
			spielerTeamAuswahl.setValue(IDPicker.pick(steuerung.getTeams(), letztesTeamSpieler));
		}
		...
		loe.setOnAction((e) -> {
			if (steuerung.getAktiveSpielerEinesTeams(spielerTeamAuswahl.getValue().getID()).size() > 0) {
				new ListDialog<Spieler>(steuerung.getAktiveSpielerEinesTeams(spielerTeamAuswahl.getValue().getID()),
						stage, "Welcher Spieler soll gelöscht werden", "Spieler löschen", (f) -> {
							Hauptfenster.getAlert(AlertType.CONFIRMATION, "ACHTUNG!", null,
									"Soll dieser Spieler wirklich gelöscht werden?", stage, ButtonType.YES,
									ButtonType.CANCEL).showAndWait().ifPresent((answer) -> {
										try {
											steuerung.removeSpieler(f.getID());
										} catch (IllegalArgumentException iae) {
											Hauptfenster
													.getAlert(AlertType.ERROR, "Fehler", null, iae.getMessage(), stage)
													.showAndWait();
										}
									});
							akt.aktualisieren();
				...
				Hauptfenster.getAlert(AlertType.ERROR, "Fehler", null, "Es sind keine Spieler vorhanden!", stage)
						.showAndWait();
			}
		});
	...
	private void aktualisiereSpielerListe(Team team, VBox spielerListeBox, GridPane spielerGP) {
	...
	private void aktualisiereSpielerDetails(Spieler spieler, GridPane spielerGP) {
		...
		spielerWechsel.setOnAction((e) -> {
			...
			new ListDialog<Team>(teams, stage, "Wohin soll der Spieler wechseln?", "Wechsel", (f) -> {
				Hauptfenster
						.getAlert(AlertType.WARNING, "ACHTUNG!", null, "Soll dieser Spieler wirklich wechseln?", stage,
								ButtonType.YES, ButtonType.NO)
						.showAndWait().filter((answer) -> (answer == ButtonType.YES)).ifPresent((answer) -> {
							try {
								steuerung.editSpieler(spieler.getVorname(), spieler.getNachname(), 0, f.getID(),
										spieler.getID());
								akt.aktualisieren();
							} catch (IllegalArgumentException iae) {
								if (iae.getMessage().charAt(0) == 'A') {
									...
									al.setContentText(
											"Der Spieler ist in einem Spiel gelistet, welches noch nicht abgeschlossen ist."
													+ " Deshalb kann er gerade nicht gewechselt werden!");
									al.setHeight(200);
									Optional<ButtonType> btn = al.showAndWait();
									if (btn.get().equals(zuSp)) {
										Spiel sp = IDPicker.pick(steuerung.getSpiele(),
												Long.valueOf(iae.getMessage().substring(1)));
										hfs = new HFSpiele(steuerung.getLSVonSpiel(sp.getID()).getKey(),
												steuerung.getLSVonSpiel(sp.getID()).getValue(), sp, stage, bp,
												steuerung, akt);
									}
								} else {
									Hauptfenster.getAlert(AlertType.ERROR, "Fehler", null, iae.getMessage(), stage)
											.showAndWait();
								}
							}
						});
	...
	private void aktualisiereSpielerTeamDetails(Team team, GridPane spielerGP) {
		...
		kapitaenSetzen.setOnAction((e) -> {
			if (steuerung.getAktiveSpielerEinesTeams(team.getID()).size() > 0) {
				new ListDialog<Spieler>(steuerung.getAktiveSpielerEinesTeams(team.getID()), stage, 300,
						"Welcher Spieler soll zum Kapitän werden?", "Kapitän bestimmen", (f) -> {
							team.setKapitaen(f);
				...
				Hauptfenster.getAlert(AlertType.ERROR, "Fehler", null, "In diesem Team ist kein Spieler eingetragen!",
						stage).showAndWait();
			}
		});
		...
		vizekapitaenSetzen.setOnAction((e) -> {
			if (steuerung.getAktiveSpielerEinesTeams(team.getID()).size() > 0) {
				new ListDialog<Spieler>(steuerung.getAktiveSpielerEinesTeams(team.getID()), stage, 300,
						"Welcher Spieler soll zum Vizekapitän werden?", "Vizekapitän bestimmen", (f) -> {
